...
class Game:
    ...
    def new_game(self):
        ...
        npc_x = self.player.x + 50
        npc_y = self.player.y

        self.npcs = [{
              "x": npc_x,
              "y": npc_y,
                "fala": "Ei! Tome cuidado lá fora!"
        }]
    ...
    def _update(self):
        ...
        for npc in self.npcs:
            if abs(self.player.x - npc["x"]) < 40 and abs(self.player.y - npc["y"]) < 40:
                if self.interact_pressed:
                    self.dialogo_atual = npc["fala"]
    ...
    def _draw(self):
        ...
        if self.state == STATE_MENU:
            ...
            # Draw NPC
            for npc in self.npcs:
                pygame.draw.rect(self.screen, (0, 255, 0), (npc["x"], npc["y"], 20, 20))
            ...
            if self.dialogo_atual:
                ...
                texto = font.render(self.dialogo_atual, True, (255, 255, 255))
                self.screen.blit(texto, (50, self.screen_h - 50))
